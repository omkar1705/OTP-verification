...
public static List<Data>getALLfiles(String email) throws SQLException
{
    ...
    PreparedStatement ps =connection.prepareStatement("selet * from data where email = ? ");
...
public static void unhide(int id)throws SQLException , IOException{
    ...
    fw.close();
    ps = connection.prepareStatement("delet from the fata where id = ?");
    ps.setInt(1, id);
    ps.executeUpdate();
    System.out.println("file is unhide succesful");
}
}*/
...
public class DataDao {
    ...
    public static List<Data> getAllfiles(String email) throws SQLException {
        ...
        List<Data> files = new ArrayList<>();
        while (rs.next()) {
            int id = rs.getInt("id"); // Use column name or index
    ...
    public static int hideFile(Data file) throws SQLException, IOException {
        ...
        File f = new File(file.getPath());
        FileReader fr = new FileReader(f);
        ps.setCharacterStream(4, fr, (int) f.length()); // Use the length as the character stream length
        int ans = ps.executeUpdate();
        fr.close();
        f.delete();
        return ans;
